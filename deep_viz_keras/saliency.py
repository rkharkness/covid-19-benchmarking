...
class SaliencyMask(object):
    ...
    def get_smoothed_mask(self, input_image, input_label, stdev_spread=.2, nsamples=50):
        ...
        for i in range(nsamples):
            ...
            total_gradients += self.get_mask(x_value_plus_noise, input_label)
...
#        input_tensors = [model['model'].input,        # placeholder for input image tensor
 #                        K.learning_phase(), # placeholder for mode (train or test) tense
#                        ]
        self.model = model['model']
        self.loss_fn = model['loss_fn']

    def get_mask(self, input_image,input_label):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = self.loss_fn(pred,np.expand_dims(input_label, axis=0))
            #print(loss)
