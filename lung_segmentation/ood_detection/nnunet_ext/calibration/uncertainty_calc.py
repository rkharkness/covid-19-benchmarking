...
def softmax_uncertainty(outputs_path, base_name, file_type='png', nr_labels=2, part=0, 
    ...
    for label in range(nr_labels):
        uncertainty_path = os.path.join(outputs_path, '{}_{}_.{}'.format(base_name, label, file_type))
        label_uncertainty = cv2.imread(uncertainty_path)[0].astype(np.float16)
...
def dropout_uncertainty(MC_outputs_path, base_name, label=1, file_type='png', norm=False,
    ...
    try:
        ...
        uncertainty_file_paths = [f for f in os.listdir(MC_outputs_path) 
            if '{}_{}'.format(base_name, label) in f]
        uncertainty_file_paths = [os.path.join(MC_outputs_path, f) 
            for f in uncertainty_file_paths]
        predictions = []
        for output in uncertainty_file_paths:
        # to deal with dicom/png images
            if file_type == 'dcm':
                prediction = "dcm" # need to update for ltht and nccid
            else:
                prediction = cv2.imread(output)[0].astype(np.float16)
...
def kl_uncertainty(outputs_path, base_name, file_type='png', nr_labels=2, part=0, norm=False,
    ...
    try:
        ...
        label_outputs = []
        for label in range(nr_labels):
            uncertainty_path = os.path.join(outputs_path, '{}_{}.{}'.format(base_name, label, file_type))
            label_output = cv2.imread(uncertainty_path)[0].astype(np.float16)
...
def temp_scaled_uncertainty(non_softmaxed_outputs_path, base_name, file_type='png', temp=1000, 
    ...
    outputs = []
    for label in range(nr_labels):
        output_path = os.path.join(non_softmaxed_outputs_path, '{}_{}.{}'.format(base_name, label, file_type))
        # to deal with dicom/png images
        if file_type == 'dcm':
            output = "dcm" # need to update for ltht and nccid
        else:
            output = cv2.imread(output_path)[0].astype(np.float16)
...
def mahalanobis_uncertainty(features_path, base_name, feature_key, 
    patch_size=[1, 480, 480], use_gaussian=False, norm=False):
