...
def nnUNet_extract_uncertainties(pred_dataset_name, task_id, fold_ix, 
    ...
    predictions_path = os.path.join(results_path, 'predictions', str(fold_ix), pred_dataset_name)
    outputs_path = os.path.join(results_path, 'outputs', str(fold_ix), pred_dataset_name)
    non_softmaxed_outputs_path = os.path.join(results_path, 'non_softmaxed_outputs', str(fold_ix), pred_dataset_name)
    MC_outputs_path = os.path.join(results_path, 'MC_outputs', task_id, str(fold_ix), pred_dataset_name)
    features_root_path = os.path.join(results_path, 'features', task_id, str(fold_ix))
    output_features_path = os.path.join(features_root_path, pred_dataset_name)
    eval_storage_path = os.path.join(os.environ.get('EVALUATION_FOLDER'), str(fold_ix))
    eval_path = os.path.join(eval_storage_path, pred_dataset_name)
    eval_path = os.path.join(os.environ.get('EVALUATION_FOLDER'), str(fold_ix), pred_dataset_name)
    #base_names = [f[:-7] for f in os.listdir(targets_path) if '.nii.gz' in f]
    base_names = [f for f in os.listdir(targets_path) if 'png' in f]
