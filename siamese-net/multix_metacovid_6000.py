...
total_data = pd.read_csv('/MULTIX/DATA/HOME/custom_data_ablation_6000.csv')
...
valid_loader = DataLoader(val_data, batch_size=8, num_workers=0,
                          pin_memory=True, shuffle=True, 
                          worker_init_fn=np.random.seed(seed))
...
def save_best_model(model, epoch):
  print("saving model ...")
  torch.save(model.state_dict(), "/home/ubuntu/multix_metacovid_6000_{}.pth".format(epoch))
...
optimizer = torch.optim.Adam(net.parameters(),lr =1e-4, weight_decay=1e-3) # l2 reg
