...
class FuseNet(Model):
    ...
    def build_model(self, image_size=480):
       ...
       # vgg_model_plus.add(Conv2D(filters=512, kernel_size=(3,3), strides=(2,2)))
        ...
        x = get_dropout(x, rate=0.7, mc=self.dropout_act)
        x = Dense(units=128, activation='relu')(x)
        x = get_dropout(x, rate=0.5, mc=self.dropout_act)
