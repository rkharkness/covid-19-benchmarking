...
class SSL_AM_Seg(Model):
    def __init__(self, pretrained=None, supervised=False):
        super(SSL_AM_Seg, self).__init__()
        self.pretrained = pretrained
        self.model_name = 'ssl_am_seg'
        self.model_type = 'keras'
        self.supervised = supervised
        self.input_size = (480,480,3)
        self.optimizer = Adam() #Adam(clipvalue=2, clipnorm=1)
        if self.supervised:
            self.loss_fn = tf.keras.losses.BinaryCrossentropy(from_logits=False, reduction=tf.keras.losses.Reduction.SUM_OVER_BATCH_SIZE)
    ...
    def unet_cbam(self, x):
        ...
        drop4 = Dropout(0.5, name='dropout_1')(conv4)
...
if __name__ == "__main__":
    model = SSL_AM(supervised=True, pretrained=True).build_model() #pretrained="/MULTIX/DATA/nccid/ssl_am_chexpert_unsupervised_1.h5").build_model()
    print(model['model'].summary())
    network = model['model'].load_weights('/MULTIX/DATA/HOME/covid-19-benchmarking/weights/ssl_am_seg/ssl_am_seg_supervised_1.h5')
