...
#      x = Conv2D(self.n_channels, (7,7), strides=(2,2))(x)
 ...
 #     x = Dropout(self.dropout)(x, training=self.dropout_act)
      ...
      x = attention_block(x, encoder_depth=3) #2

      x = residual_block(x, input_channels=64, output_channels=128, stride=2)  # 8x8
      x = attention_block(x, encoder_depth=2) #1

      x = residual_block(x, input_channels=128, output_channels=128, stride=2)  # 4x4
      x = attention_block(x, encoder_depth=1) #1

      x = residual_block(x, input_channels=128, output_channels=256)
      x = residual_block(x, input_channels=256, output_channels=256)
#      x = residual_block(x, input_channels=512, output_channels=512)
      
      x = Conv2D(256, (5,5), strides=(2,2))(x)  # 1
      x = Conv2D(256, (5,5), strides=(2,2))(x)
      x = Flatten()(x)
