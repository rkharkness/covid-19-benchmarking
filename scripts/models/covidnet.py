...
class CovidNet(nn.Module):
    def __init__(self, model='small', pretrained=None):
        super(CovidNet, self).__init__()

        #self.loss_fn = WeightedBCE()
        self.lr = 1e-4
        self.optimizer = 'adam'
        self.model_name = 'covidnet'
        ...
        if (self.pretrained == None and self.pretrained != True):
            n_classes = 5
            self.loss_fn = nn.BCELoss() #SmoothedBCE(0.0)
        else:
            n_classes = 1
        ...
        self.add_module('fc1', nn.Linear(15 * 15 * 424, 512))
