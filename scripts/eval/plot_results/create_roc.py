...
def model_pr(model):
 ...
 #   model_name_map = {'CAPSNET':'CAPSNET','XVITCOS_SEG':'XVITCOS (ROI)','SSL_AM_SEG':'SSL-AM (ROI)','CORONET_TFL_SE>
#    dataset_name = {'nccid_val':'NCCID Val','covidgr':'COVIDGR', 'nccid_test':'NCCID Test','ltht_pneumonia':'LTHT (>
...
#    data = [None] + data
    with plt.style.context(['science', 'nature']):
        f, ax = plt.subplots(1, 5, figsize=(6,3.5), gridspec_kw={'width_ratios': [2.25, 1, 1, 1, 1]})
        counter = 0
...
#            tprs_lower = lower_ci
            ...
            ax[0].plot(mean_x, mean_y, label=data_name, lw=1, alpha=.8)   #'%s: \n AUC = %0.2f $\pm$ %0.2f' % (data>
            ax[0].fill_between(mean_x, tprs_lower, tprs_upper,alpha=.1)
            ax[0].set_title(r'\textbf{{{}}}'.format(model_name_map[model_name.upper()]), rotation='vertical', x=-.3,y=0.35, fontsize=8, fontdict=dict(weight='bold'), wrap=True)
            ax[0].set_xlabel('Recall',fontsize=7.5)
            ax[0].set_ylabel('Precision',fontsize=7.5)
            ...
            a.set_title(r'\textbf{{{}}}'.format(data_name), fontsize=8, weight='bold', fontdict=dict(weight='bold'), wrap=True)
       ...
       #     a.set_xlabel('False Positive Rate',fontsize=7.5)
        ...
        f.text(0.7, 0.039, 'Recall', ha='center', fontsize=7.5)
        f.legend(handles, labels, bbox_to_anchor=(0.9, 0.00), ncol=5)
#            a.set_ylabel('True Positive Rate') #,fontsize=7)            
            
        f.tight_layout()
        f.savefig(f"/MULTIX/DATA/HOME/covid-19-benchmarking/results/{model_name}_pr.pdf",dpi=300, bbox_inches='tight')
...
#            tprs_lower = lower_ci
            ...
            ax[0].fill_between(mean_x, tprs_lower, tprs_upper,alpha=.1)
            ax[0].set_title(r'\textbf{{{}}}'.format(model_name_map[model_name.upper()]), rotation='vertical', x=-.3,y=0.35, fontsize=8, fontdict=dict(weight='bold'), wrap=True) #,fontsize=8)
            ax[0].set_xlabel('False Positive Rate',fontsize=7.5)
            ax[0].set_ylabel('True Positive Rate',fontsize=7.5)
            #ax[0].legend(fontsize=5, loc='lower right')
       ...
       #     a.set_xlabel('False Positive Rate',fontsize=7.5)
        ...
        f.text(0.7, 0.039, 'False Positive Rate', ha='center', fontsize=7.5)
        f.legend(handles, labels, bbox_to_anchor=(0.9, 0.00), ncol=5)
#            a.set_ylabel('True Positive Rate') #,fontsize=7)            
            
        f.tight_layout()
        f.savefig(f"/MULTIX/DATA/HOME/covid-19-benchmarking/results/{model_name}_roc.pdf",dpi=300, bbox_inches='tight')   
...
#    data = ['nccid_test','ltht','ltht_no_pneumonia','ltht_pneumonia','covidgr']
    with plt.style.context(['science', 'nature']):
        f, ax = plt.subplots(1, 5, figsize=(11,2.5)) #, gridspec_kw={'width_ratios': [1, 1, 1, 1, 1]})
...
#                ax[0].set_xlabel('False Positive Rate', fontsize=7.5)
 #               ax[1].set_xlabel('False Positive Rate', fontsize=7.5)  
                ax[2].set_xlabel('False Positive Rate', fontsize=7.5)  
  #              ax[3].set_xlabel('False Positive Rate', fontsize=7.5)  
   #             ax[4].set_xlabel('False Positive Rate', fontsize=7.5)  
                model_label = model_name_map[model_names[i].upper()]

                colors = plt.rcParams["axes.prop_cycle"].by_key()["color"]
                if str(mean_auc) != 'nan':
                    legend = '%s: \n AUC = %0.2f $\pm$ %0.2f' % (model_label, mean_auc, std_auc) 
                a.plot(mean_x, mean_y, lw=1, alpha=.8, color=colors[counter], marker=markers[i], markevery=10, label=legend)
...
#        plt.title(r'\textbf{{{}}}'.format(model_name_map[model_names[i].upper()]), fontsize=8)
  ...
  #      f.legend(handles, labels, bbox_to_anchor=(0.6, 0.00), ncol=2)
 #       f.ylabel('True Positive Rate',fontsize=7.5)            
            
        f.tight_layout()
        f.savefig(f"/MULTIX/DATA/HOME/covid-19-benchmarking/results/seg_new_{model_names[1]}_roc.pdf",dpi=300, bbox_inches='tight')   
