...
from modelsgenesis_pytorch.config import configs
...
class SSL_AM(Model):
    def __init__(self, pretrained_weights, supervised):
    ...
    def CBAM_attention(self, ratio, kernel_size, dr_ratio, activ_regularization):
        x = Input((480,480,3))
    ...
    def unet_cbam(self, pretrained_weights=None, input_size=(480, 480, 3), kernel_size=3, ratio=3, activ_regularization=0.01):
        inputs = tf.keras.Input(input_size)

        conv1 = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal')(inputs)
        conv1 = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv1)
        conv1 = self.CBAM_attention(conv1, ratio, kernel_size, dr_ratio, activ_regularization)

        pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)

        conv2 = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool1)
        conv2 = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv2)
        conv2 = self.CBAM_attention(conv2, ratio, kernel_size, dr_ratio, activ_regularization)

        pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)

        conv3 = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool2)
        conv3 = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv3)
        conv3 = self.CBAM_attention(conv3, ratio, kernel_size, dr_ratio, activ_regularization)

        pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)

        conv4 = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool3)
        conv4 = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv4)
        conv4 = self.CBAM_attention(conv4, ratio, kernel_size, dr_ratio, activ_regularization)
        ...
        conv6 = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal')(merge6)
        conv6 = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv6)
        conv6 = self.CBAM_attention(conv6, ratio, kernel_size, dr_ratio, activ_regularization)
        ...
        conv7 = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal')(merge7)
        conv7 = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv7)
        conv7 = self.CBAM_attention(conv7, ratio, kernel_size, dr_ratio, activ_regularization)
    ...
    def classifier(self, activ_regularization):
      #  activ_regularization=0.00001

        unet_cbam_model = self.unet()
    ...
    def build_model(self):
        if self.supervised:
            assert self.pretrained_weights
            model = self.classifier()
        else:
            model = self.unet_cbam()
